/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * RestoSmart Restaurant Management API
 * OpenAPI spec version: 0.2.0
 */

export * from "./activeDiscountStatus";
export * from "./campaign";
export * from "./campaignSegmentsResponse";
export * from "./campaignSegmentsResponseSegments";
export * from "./campaignSend";
export * from "./checklistItem";
export * from "./checkoutResult";
export * from "./createCampaignBody";
export * from "./createCampaignBodyTargetSegment";
export * from "./createCampaignBodyType";
export * from "./createCustomerBookingBody";
export * from "./createEmployeeBody";
export * from "./createEmployeeBodyStatus";
export * from "./createInventoryItemBody";
export * from "./createMenuItemBody";
export * from "./createReservationBody";
export * from "./createReservationBodySource";
export * from "./createReviewBody";
export * from "./createSaleRecordBody";
export * from "./createShiftBody";
export * from "./createShiftBodyDayOfWeek";
export * from "./customerBooking";
export * from "./customerBookingRestaurant";
export * from "./dailyAnalyticsEntry";
export * from "./dailySummary";
export * from "./deal";
export * from "./dealType";
export * from "./discountBlastRequest";
export * from "./discountOutcome";
export * from "./discountSettings";
export * from "./discountSettingsTargetType";
export * from "./dishStat";
export * from "./employee";
export * from "./employeeStatus";
export * from "./financesSummary";
export * from "./flashDealRequest";
export * from "./getPersonalizedOffersParams";
export * from "./getReviewStatsParams";
export * from "./healthStatus";
export * from "./heatmapData";
export * from "./heatmapDay";
export * from "./heatmapHourCell";
export * from "./heatmapHourLabel";
export * from "./insightSuggestion";
export * from "./inventoryItem";
export * from "./listMarketplaceRestaurantsParams";
export * from "./listMyBookingsParams";
export * from "./listPosSalesParams";
export * from "./listReservationsParams";
export * from "./listReviewsParams";
export * from "./loyaltyBalance";
export * from "./marketplaceFlashDeal";
export * from "./marketplaceFlashDealRestaurant";
export * from "./marketplaceMenuItem";
export * from "./marketplaceRestaurant";
export * from "./marketplaceRestaurantDetail";
export * from "./menuIngredient";
export * from "./menuItem";
export * from "./menuItemAnalytics";
export * from "./monthlySaleData";
export * from "./notificationRecord";
export * from "./onboardingStatus";
export * from "./overviewSummary";
export * from "./overviewSummaryActiveDiscount";
export * from "./overviewSummaryLowStockItemsItem";
export * from "./patchReservationStatusBody";
export * from "./patchReservationStatusBodyStatus";
export * from "./peakHourEntry";
export * from "./performanceAnalytics";
export * from "./personalizedOffersResponse";
export * from "./personalizedOffersResponseActiveFlashDealsItem";
export * from "./personalizedOffersResponseRecommendationsItem";
export * from "./platformSetting";
export * from "./posSale";
export * from "./recordPosSaleBody";
export * from "./replyToReviewBody";
export * from "./reservation";
export * from "./reservationSource";
export * from "./reservationStats";
export * from "./reservationStatus";
export * from "./restaurantInfo";
export * from "./retentionCustomer";
export * from "./retentionMetrics";
export * from "./review";
export * from "./reviewStats";
export * from "./reviewStatsDistribution";
export * from "./saleRecord";
export * from "./scheduledDealRequest";
export * from "./segmentInfo";
export * from "./segmentInfoCustomersItem";
export * from "./setMenuItemIngredientsBody";
export * from "./setMenuItemIngredientsBodyIngredientsItem";
export * from "./shift";
export * from "./shiftDayOfWeek";
export * from "./shiftReminder";
export * from "./sourceBreakdown";
export * from "./subscription";
export * from "./superAdminRestaurant";
export * from "./superAdminRestaurantSubscription";
export * from "./superAdminStats";
export * from "./superAdminStatsBookings";
export * from "./superAdminStatsLoyalty";
export * from "./superAdminStatsRestaurants";
export * from "./superAdminStatsReviews";
export * from "./superAdminStatsSubscriptions";
export * from "./toggleDealBody";
export * from "./updateDiscountBody";
export * from "./updateDiscountBodyTargetType";
export * from "./updateMenuItemBody";
export * from "./updateOnboardingStepBody";
export * from "./updateReservationBody";
export * from "./updateReservationBodySource";
export * from "./updateReservationBodyStatus";
export * from "./updateRestaurantInfoBody";
export * from "./updateSettingBody";
export * from "./updateSuperAdminRestaurant200";
export * from "./updateSuperAdminRestaurantBody";
export * from "./workingNowEmployee";
