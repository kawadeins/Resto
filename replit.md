...
-   **Profile Hub (Customer):** The customer `/profile` page is a rich personal social hub. The hero shows 4 stats (Buchungen, Bewertungen, Freunde, Punkte). The Übersicht tab includes: loyalty/tier card, Level/XP card (5 levels: Neuer Entdecker → Wiener Kenner, XP = bookings×10 + reviews×5 + friends×3), Quick Actions grid (4 shortcuts), Plans preview (Solo + Gruppe), Food Journey / visited restaurants (from completed bookings), Friends mini-section with avatar stack. The Sozial tab embeds the full FriendsPanel (add/manage friends, incoming requests) plus activity feed. All existing tabs (Konto, Geschmack, Verlauf, Sozial, Habits) and standalone routes (/friends, /meal-plan, /my-bookings) remain fully intact.
