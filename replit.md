...
-   **Master Brain + Auto Decision Engine v2:**
    ...
    -   **API:** GET `/api/brain/status` (26 systems, priorities, topPriorities, brainMode, readinessPercent, connectionCounts, summaryLines, autoActionsThisRun, autoActionHistory); POST `/api/brain/action/:actionType`.
    -   **Founder Interface ("Master Brain" tab):** Header with health badge + mode badge + readiness %, summary lines block, 8-stat grid, brain mode warning banner, 3 section tabs (System-Übersicht / Founder Intelligence Feed / Auto-Actions & History). Overview: scrollable 26-system card grid with connection dot indicators + detail panel (connection verification 5-box grid, metrics, performance bar, incidents). Intelligence Feed: critical items, auto-fixed items, manual review queue, connection warnings. Actions: auto-actions this run, history log, 3 manual trigger buttons. Expandable priority cards with 3-column detail view.
