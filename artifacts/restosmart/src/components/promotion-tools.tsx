...
import {
  ...
  MapPin, Lightbulb, ToggleLeft, ToggleRight,
} from "lucide-react";
...
const C = {
  ...
  gradHov:   "linear-gradient(135deg,#639aff,#8e6fff)",
  active:    "#22C55E",
  paused:    "#F59E0B",
  danger:    "#EF4444",
  text:      "#FFFFFF",
...
function GradBtn({
  onClick, disabled, children, full = true,
}: {
  onClick: () => void; disabled?: boolean; children: React.ReactNode; full?: boolean;
}) {
  return (
    <motion.button
      onClick={onClick}
      disabled={disabled}
      whileHover={disabled ? {} : { boxShadow: "0 0 22px rgba(79,140,255,0.45)", scale: 1.01 }}
      ...
      <div style={{ padding: "0 24px 24px" }} className="space-y-5">
          ...
          {relevantBoosts.map((cfg, idx) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -3, boxShadow: isLive ? "0 16px 40px rgba(0,0,0,0.45),0 0 28px rgba(34,197,94,0.18)" : "0 16px 40px rgba(0,0,0,0.45)" }}
                style={{
                  ...
                  gap: 16,
                  border: isLive ? `1px solid ${C.borderAct}` : isPaused ? `1px solid ${C.borderPau}` : `1px solid ${C.border}`,
                  boxShadow: isLive ? C.glowAct : C.shadow,
              ...
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-sm leading-snug" style={{ color: C.text }}>
                        {cfg.label}
                      </div>
                ...
                <AnimatePresence>
                  {nowActive && !isLive && (
                    ...
                    >
                      <motion.span
                        className="w-1.5 h-1.5 rounded-full"
                        style={{ backgroundColor: C.paused }}
        ...
        {budgets.length > 0 && (
          <div style={{ backgroundColor: "rgba(255,255,255,0.02)", borderRadius: 16, border: `1px solid ${C.border}`, padding: 20 }} className="space-y-4">
                  ...
                  <div key={b.id} style={{ borderRadius: 12, border: `1px solid ${C.border}`, backgroundColor: "rgba(255,255,255,0.02)", padding: 16 }} className="space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-medium flex items-center gap-2" style={{ color: C.text }}>
                        ...
                        {b.budgetExhausted && (
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: "rgba(239,68,68,0.12)", color: C.danger, border: "1px solid rgba(239,68,68,0.22)" }}>
                            Budget aufgebraucht
                          </span>
                        )}
                      </span>
                    ...
                    {isEditing && (
                      <div className="pt-3 space-y-3" style={{ borderTop: `1px solid ${C.border}` }}>
                          ...
                          {[0, 5, 10, 20, 50].map(amount => (
                            <motion.button
                              ...
                              style={{
                                ...
                                color: budgetInput[b.id] === String(amount) ? "#fff" : C.muted,
                                cursor: "pointer",
                              }}
                            >
                        ...
                        <div className="flex gap-2">
                          ...
                          />
                          <motion.button
                            whileHover={{ boxShadow: "0 0 16px rgba(79,140,255,0.35)" }}
                            whileTap={{ scale: 0.97 }}
                            onClick={() => {
                            ...
                            style={{
                              ...
                              fontSize: 13,
                              cursor: "pointer",
                              opacity: budgetMutation.isPending ? 0.6 : 1,
                            }}
                          >
                            Speichern
                          </motion.button>
            ...
            <div className="flex items-start gap-2.5 text-[11px] rounded-xl px-3 py-2.5" style={{ color: C.muted, backgroundColor: "rgba(79,140,255,0.07)", border: "1px solid rgba(79,140,255,0.14)" }}>
  ...
  return (
    ...
    >
      <div style={{ padding: 20 }} className="space-y-5">
        ...
        {/* AI suggestion */}
        <div className="flex items-start gap-2.5 rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(79,140,255,0.07)", border: "1px solid rgba(79,140,255,0.15)" }}>
        ...
        <div style={{ borderRadius: 12, border: `1px solid ${C.border}`, backgroundColor: "rgba(255,255,255,0.02)", padding: "12px 14px" }} className="flex items-center justify-between gap-3">
          ...
          <motion.button
            ...
            style={{
              ...
              fontSize: 12,
              cursor: "pointer",
              opacity: autoOptMutation.isPending ? 0.6 : 1,
              flexShrink: 0,
            }}
          >
